# Version 0.17 (Apr 2020)
  ...
  # Version 0.17 (Apr 2020)
  # ------------
  #   Modified the function Default.inits.local (now calling inits.BetweenWorker)
  #   Added the argument use.mle.as.inits in the model.BetweenWorker fct arguments
  #
  # Version 0.16
  # ------------
  #   Commented out the definition of b.min
  #
  # Version 0.15
  # ------------
  ...
  use.mle.as.inits=T)
{
  ...
  # modif_0.17
  #   changed the name of argument log.sigma.within.mu -> log.sw.mu 
  #   added the argument start.with.mle + sw.range, sb.range, log.sw.prec, log.sb.mu & log.sb.prec)
  # modif_0.13


  Default.inits.local <- function(data, logNormal.distrn, log.sw.mu, use.uniform.prior.on.sds, include.censored.data=F, 
                                  start.with.mle=F, 
                                  sw.range=sigma.within.range, sb.range=sigma.between.range,
                                  log.sw.prec=log.sigma.within.prec, 
                                  log.sb.mu=log.sigma.between.mu, log.sb.prec=log.sigma.between.prec)
  {
    converged <- F # new_0.17
    ...
    if (start.with.mle)
    {
      ...
      if (converged)
      {
        ...
        normalized.y <- data$y # not as fancy (not taking the log) as in the next if-block, but good enough!
      }
    }
    ...
    if (!converged)
    {   
      # modif_0.17 The entire content of the previous version of Default.inits.local
      # was moved to this if-block
      ...
      if (length(sigma.within) == 0 && !include.censored.data)
      {
        ...
        # modif_0.17 
        #   - changed name of argument log.sigma.within.mu -> log.sw.mu
        #   - added use of argument start.with.mle, sw.range, sb.range, log.sw.* and log.sb.* 
        
        tmp <- Default.inits.local(data, logNormal.distrn, log.sw.mu, use.uniform.prior.on.sds, include.censored.data=T, 
                 start.with.mle=F, sw.range=sw.range, sb.range=sb.range,
                 log.sw.prec=log.sw.prec, log.sb.mu=log.sb.mu, log.sb.prec=log.sb.prec)
      ...
      if (length(sigma.within) == 0)
      {
        ...
        {
          sigma.within <- mean(sw.range) # modif_0.17 changed sigma.within.range for sw.range
        }
        else
        {
          sigma.within <- exp(log.sw.mu) # modif_0.17 changed log.sigma.within.mu -> log.sw.mu
        }
      }
    }
  ...
  # modif_0.17 added use of argument start.with.mle
  # modif_0.12 
  # new_0.11
  default.inits <- Default.inits.local(data, outcome.is.logNormally.distributed, log.sigma.within.mu, use.uniform.prior.on.sds, start.with.mle=use.mle.as.inits)
