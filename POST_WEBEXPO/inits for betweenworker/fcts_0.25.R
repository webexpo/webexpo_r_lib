...
dens.gen.icdf <- function(o, A=o$A, range=numeric(0), u=runif(1), start=mean(range), precision=1e-6, inestimable.lower.limit=F, NR.max.iter=200, save.objects=character(0), unlink=T)
{
  ...
  ref.points <- function(o, A, start, range, inestimable.lower.limit=F, f.ratio.remote=1e-8, epsilon=1e-6, max.niter=NR.max.iter, find.remote=T)
  {
    # Arguments:
    # ----------------------------------------------
    ...
    # modif_0.25 Added the argument find.remote to the function


    # modif_0.25 The code in the fct below above "Run pure N-R" is new (replacing old [removed] code)
    # new_0.11
    ...
    if (!mode$found)
    {
      ...
      nr <- secured.NR.search(o.mode, A, start, epsilon, range=range, max.niter=max.niter, inestimable.lower.limit=inestimable.lower.limit) # modif_0.25 added argument inestimable.lower.limit
      ...
      if (nr$converged)
      {
        ...
        if (hs > 0)
        {
          ...
          mode$x <- higher.local.max(o, A, nr$x, range, hs, epsilon) # modif_0.25 Added argument hs
          bimodal <- T # new_0.25
        }
        else
        {
    ...
    # modif_0.25 The block of code below existed in earlier versions but it now embedded in an if-condition 
    ...
    {
      # new_0.25 // we were not looking for remote limits
      remote.left <- NA
      remote.right <- NA
      log.f.ratio.remote <- NA
    } 
    
    # new_0.25 added bimodal to the returned list
      ...
      while (!accepted)
      {
        ...
        else
        {
          ...
          if (x <= limit$x[1])
          {
            ...
            {
              # not limit$usable[1]
              x <- (min(bounds$x, na.rm=T) + limit$x[1]) / 2 # modif_0.25 added na.rm=T
            }
          }
          else
          {
  ...
  if (ref$mode$found & !bimodal)
  {
    ...
    ref.alt <- ref.points(o, A, start=ref.alt.range.r[2], range=ref.alt.range.r, epsilon=precision, find.remote=F)
    ...
    if (!bimodal)
    {
      ...
      ref.alt <- ref.points(o, A, start=ref.alt.range.l[1], range=ref.alt.range.l, epsilon=precision, find.remote=F)
  ...
  # modif_0.25 In the condition below: o$potentially.bimodal was changed for 'bimodal'
